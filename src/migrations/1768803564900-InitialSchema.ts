...
export class InitialSchema1768803564900 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        const safeCreateTable = async (query: string, tableName: string) => {
          try {
        ...
        // Helper function to safely create type/enum
        const safeCreateType = async (query: string, typeName: string) => {
          try {
            await queryRunner.query(query);
          } catch (error: any) {
            if (error.message?.includes('already exists')) {
              console.log(`Type "${typeName}" already exists, skipping...`);
            } else {
              throw error;
            }
          }
        ...
        const safeCreateIndex = async (query: string, indexName: string) => {
          try {
            await queryRunner.query(query);
          } catch (error: any) {
            if (error.message?.includes('already exists')) {
        ...
        const safeAddConstraint = async (query: string, constraintName: string) => {
          try {
            await queryRunner.query(query);
          } catch (error: any) {
            if (error.message?.includes('already exists')) {
        ...
        // Create tags table first (it's referenced by other tables)
        ...
        // Create enums
        await safeCreateType(`CREATE TYPE "public"."tasks_type_enum" AS ENUM('follow_up_call', 'email_follow_up', 'appointment_reminder', 'treatment_follow_up', 'loyalty_reward', 'general')`, 'tasks_type_enum');
        await safeCreateType(`CREATE TYPE "public"."tasks_status_enum" AS ENUM('pending', 'in_progress', 'completed', 'cancelled')`, 'tasks_status_enum');
        ...
        // Create leads enum and table
        await safeCreateType(`CREATE TYPE "public"."leads_status_enum" AS ENUM('new', 'contacted', 'qualified', 'appointment_scheduled', 'converted', 'lost', 'follow_up', 'merged')`, 'leads_status_enum');
        await safeCreateTable(
        ...
        // Create appointments enum and table
        await safeCreateType(`CREATE TYPE "public"."appointments_status_enum" AS ENUM('pending', 'confirmed', 'in_progress', 'completed', 'cancelled', 'no_show')`, 'appointments_status_enum');
        await safeCreateTable(
        ...
        // Create users enum and table
        await safeCreateType(`CREATE TYPE "public"."users_role_enum" AS ENUM('admin', 'SUPER_ADMIN', 'clinic_owner', 'doctor', 'secretariat', 'salesperson', 'client')`, 'users_role_enum');
        await safeCreateTable(
        ...
        await safeCreateType(`CREATE TYPE "public"."communication_logs_type_enum" AS ENUM('call', 'email', 'sms', 'whatsapp', 'meeting', 'note')`, 'communication_logs_type_enum');
        await safeCreateType(`CREATE TYPE "public"."communication_logs_direction_enum" AS ENUM('outgoing', 'incoming', 'missed')`, 'communication_logs_direction_enum');
        await safeCreateType(`CREATE TYPE "public"."communication_logs_status_enum" AS ENUM('completed', 'missed', 'no_answer', 'voicemail', 'scheduled', 'cancelled')`, 'communication_logs_status_enum');
        await safeCreateTable(
        ...
        await safeCreateTable(
          `CREATE TABLE "customer_tags" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "customerId" uuid NOT NULL, "tagId" uuid NOT NULL, "addedBy" uuid, "notes" text, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "PK_339dae325423407628d43e023bd" PRIMARY KEY ("id"))`,
          'customer_tags'
        );
    
        // Create ad_campaigns table
        await safeCreateTable(
          `CREATE TABLE "ad_campaigns" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "platform" character varying(50) NOT NULL, "channel" character varying(255) NOT NULL, "externalId" character varying(255) NOT NULL, "ownerAgentId" uuid, "name" character varying(255) NOT NULL, "budget" numeric(12,2) NOT NULL DEFAULT '0', "startDate" TIMESTAMP WITH TIME ZONE, "endDate" TIMESTAMP WITH TIME ZONE, "metadata" jsonb, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "PK_7877713eb87f782dd190eed85a7" PRIMARY KEY ("id"))`,
          'ad_campaigns'
        );
    
        // Create indexes for ad_campaigns
        await safeCreateIndex(`CREATE INDEX "IDX_389771d6eb1e44bb0b562e6237" ON "ad_campaigns" ("platform")`, 'IDX_389771d6eb1e44bb0b562e6237');
        await safeCreateIndex(`CREATE INDEX "IDX_1310aa7cd0f2519090f899ff1e" ON "ad_campaigns" ("externalId")`, 'IDX_1310aa7cd0f2519090f899ff1e');
        await safeCreateIndex(`CREATE INDEX "IDX_45c23fb387d541458f8e996035" ON "ad_campaigns" ("ownerAgentId")`, 'IDX_45c23fb387d541458f8e996035');
        ...
        await safeCreateIndex(`CREATE INDEX "IDX_62fd8c2a24d920f1f23aa312e3" ON "audit_logs" ("resource", "createdAt")`, 'IDX_62fd8c2a24d920f1f23aa312e3');
        await safeCreateIndex(`CREATE INDEX "IDX_99e589da8f9e9326ee0d01a028" ON "audit_logs" ("userId", "createdAt")`, 'IDX_99e589da8f9e9326ee0d01a028');
    
        // Create consent_records table
        await safeCreateTable(
          `CREATE TABLE "consent_records" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "userId" uuid NOT NULL, "consentType" character varying NOT NULL, "version" character varying NOT NULL, "granted" boolean NOT NULL, "metadata" json, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "PK_030be152eaa44998166470b2ccc" PRIMARY KEY ("id"))`,
          'consent_records'
        );
    
        // Create appointment_holds table
        await safeCreateTable(
          `CREATE TABLE "appointment_holds" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "clinicId" character varying NOT NULL, "serviceId" character varying NOT NULL, "providerId" character varying NOT NULL, "startTime" TIMESTAMP WITH TIME ZONE NOT NULL, "endTime" TIMESTAMP WITH TIME ZONE NOT NULL, "expiresAt" TIMESTAMP WITH TIME ZONE NOT NULL, "clientId" character varying, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "PK_43a358fb879ff7a7a878262752a" PRIMARY KEY ("id"))`,
          'appointment_holds'
        );
    
        // Create lead_tags junction table
        await safeCreateTable(
          `CREATE TABLE "lead_tags" ("leadId" uuid NOT NULL, "tagId" uuid NOT NULL, CONSTRAINT "PK_130f0edfe14b6ce6a8d238eb84a" PRIMARY KEY ("leadId", "tagId"))`,
          'lead_tags'
        );
    
        // Create indexes for lead_tags
        await safeCreateIndex(`CREATE INDEX "IDX_113b3f1b26fc2e3deb2eb9fe1a" ON "lead_tags" ("leadId")`, 'IDX_113b3f1b26fc2e3deb2eb9fe1a');
        await safeCreateIndex(`CREATE INDEX "IDX_ba17e62b98176a43e1703b9b45" ON "lead_tags" ("tagId")`, 'IDX_ba17e62b98176a43e1703b9b45');
    
        // Add foreign key constraints
        await safeAddConstraint(`ALTER TABLE "tasks" ADD CONSTRAINT "FK_9a16d2c86252529f622fa53f1e3" FOREIGN KEY ("assigneeId") REFERENCES "users"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`, 'FK_9a16d2c86252529f622fa53f1e3');
        await safeAddConstraint(`ALTER TABLE "tasks" ADD CONSTRAINT "FK_27b517d1d7a2efe28177e98fdba" FOREIGN KEY ("customerId") REFERENCES "leads"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`, 'FK_27b517d1d7a2efe28177e98fdba');
        await safeAddConstraint(`ALTER TABLE "leads" ADD CONSTRAINT "FK_3a30b5301edbd2e57e032961e9f" FOREIGN KEY ("assignedSalesId") REFERENCES "users"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`, 'FK_3a30b5301edbd2e57e032961e9f');
        await safeAddConstraint(`ALTER TABLE "services" ADD CONSTRAINT "FK_72299eb2e42a1f560d44be97d04" FOREIGN KEY ("clinicId") REFERENCES "clinics"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`, 'FK_72299eb2e42a1f560d44be97d04');
        await safeAddConstraint(`ALTER TABLE "clinics" ADD CONSTRAINT "FK_3117a13727ec3247fa47cc40e95" FOREIGN KEY ("ownerId") REFERENCES "users"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`, 'FK_3117a13727ec3247fa47cc40e95');
        await safeAddConstraint(`ALTER TABLE "appointments" ADD CONSTRAINT "FK_040a7ee2072d45bc98efddf3c02" FOREIGN KEY ("clinicId") REFERENCES "clinics"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`, 'FK_040a7ee2072d45bc98efddf3c02');
        await safeAddConstraint(`ALTER TABLE "appointments" ADD CONSTRAINT "FK_f77953c373efb8ab146d98e90c3" FOREIGN KEY ("serviceId") REFERENCES "services"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`, 'FK_f77953c373efb8ab146d98e90c3');
        await safeAddConstraint(`ALTER TABLE "appointments" ADD CONSTRAINT "FK_2428e01f899c4edb909e8798b63" FOREIGN KEY ("providerId") REFERENCES "users"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`, 'FK_2428e01f899c4edb909e8798b63');
        await safeAddConstraint(`ALTER TABLE "appointments" ADD CONSTRAINT "FK_c4dbd8eb292b83b5dc67be3cf45" FOREIGN KEY ("clientId") REFERENCES "users"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`, 'FK_c4dbd8eb292b83b5dc67be3cf45');
        await safeAddConstraint(`ALTER TABLE "loyalty_ledger" ADD CONSTRAINT "FK_7cf5739ef8c93bdaa98b5dca07e" FOREIGN KEY ("clientId") REFERENCES "users"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`, 'FK_7cf5739ef8c93bdaa98b5dca07e');
        await safeAddConstraint(`ALTER TABLE "loyalty_ledger" ADD CONSTRAINT "FK_5bfebb90af9317731de682e6673" FOREIGN KEY ("clinicId") REFERENCES "clinics"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`, 'FK_5bfebb90af9317731de682e6673');
        await safeAddConstraint(`ALTER TABLE "notifications" ADD CONSTRAINT "FK_db873ba9a123711a4bff527ccd5" FOREIGN KEY ("recipientId") REFERENCES "users"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`, 'FK_db873ba9a123711a4bff527ccd5');
        await safeAddConstraint(`ALTER TABLE "users" ADD CONSTRAINT "FK_1142607b5a447cd5ce23ef7798f" FOREIGN KEY ("referredById") REFERENCES "users"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`, 'FK_1142607b5a447cd5ce23ef7798f');
        await safeAddConstraint(`ALTER TABLE "communication_logs" ADD CONSTRAINT "FK_4992b11665bf79df55c6afde4da" FOREIGN KEY ("customerId") REFERENCES "users"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`, 'FK_4992b11665bf79df55c6afde4da');
        await safeAddConstraint(`ALTER TABLE "communication_logs" ADD CONSTRAINT "FK_fcd6b2cb16b4efdd745e426a021" FOREIGN KEY ("salespersonId") REFERENCES "users"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`, 'FK_fcd6b2cb16b4efdd745e426a021');
        await safeAddConstraint(`ALTER TABLE "customer_tags" ADD CONSTRAINT "FK_8b2101f9c8689c8ec8a0e4469c2" FOREIGN KEY ("customerId") REFERENCES "customer_records"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`, 'FK_8b2101f9c8689c8ec8a0e4469c2');
        await safeAddConstraint(`ALTER TABLE "customer_tags" ADD CONSTRAINT "FK_d52f7d2360abb04a9a966a6a268" FOREIGN KEY ("tagId") REFERENCES "tags"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`, 'FK_d52f7d2360abb04a9a966a6a268');
        await safeAddConstraint(`ALTER TABLE "customer_tags" ADD CONSTRAINT "FK_4f77ce6ee0977b68f3ec8b3cfe2" FOREIGN KEY ("addedBy") REFERENCES "users"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`, 'FK_4f77ce6ee0977b68f3ec8b3cfe2');
        await safeAddConstraint(`ALTER TABLE "ad_attributions" ADD CONSTRAINT "FK_197fb1f1c0a0e6110c378131e26" FOREIGN KEY ("customerRecordId") REFERENCES "customer_records"("id") ON DELETE CASCADE ON UPDATE NO ACTION`, 'FK_197fb1f1c0a0e6110c378131e26');
        await safeAddConstraint(`ALTER TABLE "ad_attributions" ADD CONSTRAINT "FK_cef8e1c355b902ed1b9f6cfec53" FOREIGN KEY ("adCampaignId") REFERENCES "ad_campaigns"("id") ON DELETE CASCADE ON UPDATE NO ACTION`, 'FK_cef8e1c355b902ed1b9f6cfec53');
        await safeAddConstraint(`ALTER TABLE "customer_records" ADD CONSTRAINT "FK_dd1116d15dae5959ad992a6707d" FOREIGN KEY ("customerId") REFERENCES "users"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`, 'FK_dd1116d15dae5959ad992a6707d');
        await safeAddConstraint(`ALTER TABLE "customer_records" ADD CONSTRAINT "FK_2809b570ecddeed52cd2d4058e7" FOREIGN KEY ("assignedSalespersonId") REFERENCES "users"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`, 'FK_2809b570ecddeed52cd2d4058e7');
        await safeAddConstraint(`ALTER TABLE "crm_actions" ADD CONSTRAINT "FK_0d50d9c8f5727b366a284104b71" FOREIGN KEY ("customerId") REFERENCES "customer_records"("id") ON DELETE CASCADE ON UPDATE NO ACTION`, 'FK_0d50d9c8f5727b366a284104b71');
        await safeAddConstraint(`ALTER TABLE "crm_actions" ADD CONSTRAINT "FK_7b670798feb3196663f9a581cc9" FOREIGN KEY ("salespersonId") REFERENCES "users"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`, 'FK_7b670798feb3196663f9a581cc9');
        await safeAddConstraint(`ALTER TABLE "consent_records" ADD CONSTRAINT "FK_325a36bd104daa11a3d4542afa9" FOREIGN KEY ("userId") REFERENCES "users"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`, 'FK_325a36bd104daa11a3d4542afa9');
        await safeAddConstraint(`ALTER TABLE "lead_tags" ADD CONSTRAINT "FK_113b3f1b26fc2e3deb2eb9fe1a2" FOREIGN KEY ("leadId") REFERENCES "leads"("id") ON DELETE CASCADE ON UPDATE CASCADE`, 'FK_113b3f1b26fc2e3deb2eb9fe1a2');
        await safeAddConstraint(`ALTER TABLE "lead_tags" ADD CONSTRAINT "FK_ba17e62b98176a43e1703b9b45a" FOREIGN KEY ("tagId") REFERENCES "tags"("id") ON DELETE CASCADE ON UPDATE CASCADE`, 'FK_ba17e62b98176a43e1703b9b45a');
      }
