...
export class CrmService {
  ...
  async handleFacebookWebhook(webhookData: FacebookWebhookDto): Promise<void> {
    // Facebook sends: entry[] → changes[] → { field: 'leadgen', value: { leadgen_id } }
    // We must respond fast (< 20 s) — process asynchronously
    for (const entry of webhookData.entry) {
      ...
      for (const change of entry.changes) {
        ...
        try {
          ...
          if (duplicateCheck.isDuplicate && duplicateCheck.existingCustomer) {
            console.log(`[FB Webhook] Duplicate found — updating existing customer`);
            await this.updateExistingCustomerWithFacebookLead(
              duplicateCheck.existingCustomer, parsedLead, leadData,
            );
          } else {
            console.log(`[FB Webhook] New lead — creating record`);
            await this.createLeadFromFacebook(parsedLead, leadData);
          }
        } catch (error) {
          console.error(`[FB Webhook] Error processing leadgen_id ${leadgenId}:`, error);
  ...
  async getFacebookPageForms() {
    return this.facebookService.getPageForms();
  }
