...
if (-not $content.Contains("seedMockCrmData()")) {
  ...
  async seedMockCrmData(): Promise<any> {
    ...
    const serviceRepo = this.dataSource.getRepository(require('../clinics/entities/service.entity').Service);
    const services = await serviceRepo.find();

    const results = { leads: 0, calls: 0, appointments: 0 };
