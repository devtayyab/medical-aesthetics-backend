...
const ClinicLayout: React.FC = () => {
 ...
 const closeSidebar = () => setIsSidebarOpen(false);
  ...
  <h1 className="text-xl font-black uppercase tracking-tighter text-gray-900 notranslate">
  ...
  {/* User Profile & Logout */}
  <div className="p-5 m-4 bg-gray-50 rounded-2xl border border-gray-100 flex flex-col gap-4">
    {/* Language Switcher */}
  ...
  <div className="size-9 bg-white shadow-sm rounded-xl flex items-center justify-center shrink-0 notranslate">
  <span className="text-black font-black text-xs">
  {user?.firstName?.[0]}{user?.lastName?.[0]}
  </span>
  </div>
  <div className="flex-1 min-w-0">
  <p className="font-black text-[9px] uppercase tracking-tighter text-gray-900 truncate notranslate">
  {user?.firstName} {user?.lastName}
  </p>
  <p className="text-[8px] font-bold text-gray-400 uppercase tracking-tight truncate leading-none mt-1 notranslate">{user?.role?.replace('_', ' ')}</p>
  </div>
  </div>
 <button
