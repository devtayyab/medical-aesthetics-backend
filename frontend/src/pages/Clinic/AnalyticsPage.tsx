...
const AnalyticsPage: React.FC = () => {
 ...
 <p className="text-[10px] font-black uppercase tracking-widest text-gray-400">Synthesizing clinical data metrics...</p>
 </div>
 );
 }
 ...
 <span className="text-[9px] font-black uppercase tracking-[0.2em] text-gray-400">Operational Intelligence</span>
 </div>
 <div className="space-y-1">
 <h1 className="text-3xl md:text-5xl font-black uppercase tracking-tighter leading-none text-gray-900">Analysis & Insights</h1>
 ...
 <h2 className="text-lg font-black uppercase tracking-tighter text-gray-900">Yield by Service Category</h2>
 </div>
 ...
 <div>
 <p className="text-[11px] font-black uppercase tracking-tighter text-gray-900 leading-none mb-1">{item.serviceName}</p>
 <p className="text-[7px] font-black uppercase tracking-[0.2em] text-gray-400">{item.count} Transactions</p>
 </div>
 <div className="text-right">
 <p className="text-base font-black tracking-tighter text-black leading-none mb-1">€{item.revenue.toLocaleString()}</p>
 ...
 <h2 className="text-[15px] font-black uppercase tracking-tighter text-white">Loyalty Ecosystem</h2>
 </div>

 <div className="space-y-8">
 <div>
 <p className="text-4xl font-black tracking-tighter text-[#CBFF38] leading-none mb-1">
 {(loyaltyAnalytics?.totalPoints || 0).toLocaleString()}
 </p>
 <p className="text-[8px] font-black uppercase tracking-[0.2em] text-gray-500">Total Protocol Credit Issued</p>
 </div>
 <div className="h-px bg-white/10 w-full" />
 <div className="grid grid-cols-2 gap-4">
 <div>
 <p className="text-xl font-black text-white leading-none mb-1">{loyaltyAnalytics?.uniqueClients || 0}</p>
 <p className="text-[8px] font-black uppercase text-gray-500">Active Nodes</p>
 </div>
 <div>
 <p className="text-xl font-black text-[#CBFF38] leading-none mb-1">{(loyaltyAnalytics?.avgPointsPerTransaction || 0).toFixed(0)}</p>
...
const PremiumStatCard = ({ title, value, subValue, icon, highlight }: any) => (
 ...
 <div className="text-[7px] font-black text-gray-300 uppercase tracking-widest">{title}</div>
 </div>
 <div>
 <h3 className="text-2xl font-black uppercase tracking-tighter text-gray-900 leading-none mb-1 transition-all group-hover:translate-x-1">{value}</h3>
 <p className="text-[8px] font-black uppercase tracking-[0.1em] text-gray-400">{subValue}</p>
 </div>
 </div>
);
