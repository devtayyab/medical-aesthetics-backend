...
const TIMEZONE_OPTIONS = (() => {
  const list = (() => {
    try {
      ...
      console.error("Failed to get supported timezones from Intl", e);
    }
    return [
      "Europe/Athens",
      "Europe/Berlin",
  ...
  const options = list.map(tz => {
    try {
      ...
      return { value: tz, label: tz, offsetMin: 0 };
    }
  });

  // Sort by offset minutes, then by value (alphabetical)
  options.sort((a, b) => {
...
export const Clinics: React.FC = () => {
  ...
  const handleOpenModal = (clinic?: Clinic) => {
    if (clinic) {
      ...
      setFormData({
        ...clinic,
        timezone: clinic.timezone || "Europe/Athens",
              ...
              {activeTab === 'hours' && (
                <div className="space-y-6">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-4 bg-gray-50 rounded-2xl border border-gray-100">
                    <div>
                      <h4 className="font-bold text-gray-900">Clinic Timezone</h4>
                      <p className="text-xs text-gray-500">Select local timezone for appointments scheduling</p>
                    </div>
                    <div className="w-full md:w-80">
                      <select
                        className="w-full p-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#CBFF38] outline-none bg-white font-medium text-gray-700"
