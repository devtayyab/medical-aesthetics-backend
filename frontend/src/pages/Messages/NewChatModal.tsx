    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-10">
            ...
            <motion.div 
                ...
                className="bg-white rounded-[48px] w-full max-w-2xl shadow-[0_0_100px_rgba(0,0,0,0.5)] overflow-hidden relative z-10 flex flex-col h-[700px] border border-white/20"
            >
                {/* Header Section */}
                            ...
                            {isLoading ? (
                                ...
                                results.map((user, idx) => (
                                    <motion.button
                                        ...
                                        className="w-full p-6 bg-white rounded-[32px] border border-gray-100 flex items-center gap-6 hover:bg-black hover:text-[#CBFF38] group transition-all shadow-sm hover:shadow-2xl hover:-translate-y-1"
                                    >
                                        <div className="size-16 rounded-2xl bg-gray-50 text-gray-400 flex items-center justify-center font-black text-xl italic border border-gray-100 group-hover:bg-[#CBFF38] group-hover:text-black group-hover:rotate-6 transition-all shadow-inner">
                                        ...
                                        <div className="text-left flex-1 min-w-0">
                                            ...
                                            <div className="flex items-center gap-3">
                                               <span className="text-[9px] font-black uppercase tracking-widest text-[#CBFF38] italic bg-black/5 px-2 py-0.5 rounded group-hover:bg-[#CBFF38]/20 group-hover:text-[#CBFF38] transition-colors">{user.role?.replace('_', ' ')}</span>
                                               <span className="text-[10px] font-bold text-gray-400 truncate group-hover:text-gray-500">{user.email}</span>
                                            </div>
                                        </div>
                                        <div className="size-10 rounded-full border border-gray-100 flex items-center justify-center group-hover:bg-[#CBFF38] group-hover:text-black group-hover:border-[#CBFF38] transition-all opacity-0 group-hover:opacity-100 translate-x-4 group-hover:translate-x-0">
                                            <ArrowRight size={20} />
                                        </div>
                                    </motion.button>
                                ))
                ...
                <footer className="p-8 bg-black/5 flex items-center justify-center shrink-0">
                    <p className="text-[9px] font-black uppercase tracking-widest text-gray-400 italic flex items-center gap-2">
                       <UserPlus size={12} className="text-[#CBFF38]" />
                       Multi-Participant channels available via team management protocols
                    </p>
                </footer>
            </motion.div>
        </div>
