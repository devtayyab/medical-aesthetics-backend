...
// Re-use AppointmentCard from AppointmentCalendar for now, or copy it over later if needed.
// We should probably use the same AppointmentCard from AppointmentCalendar if we don't need changes.
import { AppointmentCard } from '../AppointmentCalendar/AppointmentCard';
import { getClinicLocalDate, getClinicLocalTime } from './useStaffDiaryData';
import type { CalendarView, CalendarAppointment, SlotClickPayload, ProviderResource } from './types';
...
export const StaffCalendarGrid: React.FC<CalendarGridProps> = ({
  viewDate,
  viewMode,
  appointments,
  blockedSlots,
  ...
  const getBlockedByDay = (day: Date): any[] => {
    ...
    return blockedSlots.filter(slot => {
      ...
      // getClinicLocalDate already returns a 'yyyy-MM-dd' string — do NOT wrap it in
      // format() (which expects a Date and silently mis-parses the string).
      const slotDateStr = getClinicLocalDate(slot.startTime, tz);
