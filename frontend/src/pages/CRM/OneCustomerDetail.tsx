...
export const OneCustomerDetail: React.FC<OneCustomerDetailProps> = ({
 ...
 <div className="text-slate-400 text-[11px] font-medium">No tags added</div>
 )}
 </div>
 </div>
 ...
 About this contact {isEditingCore && <span className="text-blue-500 font-medium">(Editing)</span>}
 </h3>
 <div className="flex items-center gap-3">
 {isEditingCore ? (
 <button 
 onClick={(e) => { e.stopPropagation(); handleSaveCoreProperties(); }} 
 className="text-[10px] bg-blue-600 hover:bg-blue-700 text-white px-2 py-1 rounded shadow-sm font-bold transition-all"
 >
 Save
 </button>
 ) : (
 ...
 className="text-[10px] text-slate-400 hover:text-slate-800 font-bold transition-colors"
 >
 Edit
 </button>
 )}
 ...
 {isEditingCore ? (
 <input type="text" className="w-full text-xs font-bold text-slate-800 border-b border-blue-300 focus:outline-none focus:border-blue-600 bg-transparent py-1" value={editedProperties.firstName || ''} onChange={(e) => setEditedProperties({...editedProperties, firstName: e.target.value})} />
 ) : (
 ...
 {isEditingCore ? (
 <input type="text" className="w-full text-xs font-bold text-slate-800 border-b border-blue-300 focus:outline-none focus:border-blue-600 bg-transparent py-1" value={editedProperties.lastName || ''} onChange={(e) => setEditedProperties({...editedProperties, lastName: e.target.value})} />
 ) : (
 ...
 {isEditingCore ? (
 <input type="text" className="w-full text-xs font-bold text-slate-800 border-b border-blue-300 focus:outline-none focus:border-blue-600 bg-transparent py-1" value={editedProperties.phone || ''} onChange={(e) => setEditedProperties({...editedProperties, phone: e.target.value})} />
 ) : (
 ...
 <div className="group relative">
 <span className="block text-[9px] font-black uppercase tracking-widest text-blue-500 mb-0.5">Clinic (Ιατρείο)</span>
 {isEditingCore ? (
 <input type="text" placeholder="Add manual clinic..." className="w-full text-xs font-bold text-slate-800 border-b border-blue-300 focus:outline-none focus:border-blue-600 bg-transparent py-1" value={editedProperties.metadata?.manualClinic || ''} onChange={(e) => setEditedProperties({...editedProperties, metadata: { ...editedProperties.metadata, manualClinic: e.target.value }})} />
 ) : (
 <span className="text-xs font-bold text-slate-800 group-hover:bg-slate-50 p-1 -ml-1 rounded transition-colors block">{customer.metadata?.manualClinic || 'Not specified'}</span>
 )}
 </div>
 <div className="group relative">
 ...
 <span className="block text-[9px] font-black uppercase tracking-widest text-slate-400 mb-0.5">Form Comments / Notes</span>
 <div className="text-xs font-medium text-slate-700 bg-slate-50 border border-slate-100 p-2 rounded-lg mt-1 whitespace-pre-wrap max-h-32 overflow-y-auto custom-scrollbar">
 {customer.notes}
 </div>
 </div>
 )}
 ...
 <div className="text-center p-6 text-xs font-medium text-slate-400">No appointments booked.</div>
 ) : (
 <div className="space-y-1">
