...
// Helper to extract hour and minute in the clinic's timezone.
// Uses Intl.DateTimeFormat with hourCycle:'h23' — the most reliable cross-browser
// 24-hour format that never produces '24:00' or AM/PM ambiguity.
...
const getClinicLocalDate = (dateStr: string, timezone?: string) => {
  const d = new Date(dateStr);
  const tz = sanitizeTz(timezone);
  try {
...
const createClinicUTCDateTime = (localDate: Date, timeStr: string, timezone: string): Date => {
    const tz = sanitizeTz(timezone);
    // Build an ISO string that represents the desired local date+time.
    // Use local getters on localDate since localDate is constructed from a date-only string
    // or passed from the UI where local date values represent the user's intent.
    const yyyy = localDate.getFullYear();
    const mm = String(localDate.getMonth() + 1).padStart(2, '0');
    const dd = String(localDate.getDate()).padStart(2, '0');
    const isoString = `${yyyy}-${mm}-${dd}T${timeStr}:00`;
    // Start with UTC interpretation of that local-looking string
    ...
    try {
      ...
      // Iteratively correct: find a UTC moment whose clinic-local representation equals isoString
      for (let i = 0; i < 4; i++) {
      ...
      // Fallback if timezone is invalid — treat as UTC
...
export const SalesWeekCalendar: React.FC = () => {
  ...
  const handleCreateBooking = async () => {
    ...
    if (!clientId || wizardServices.length === 0 || !wizardClinic) {
      setIsSubmitting(false);
      return;
    }
                  ...
                  {!isWalkIn ? (
                    <div className="space-y-3">
                      ...
                      <div className="space-y-2 max-h-[180px] overflow-y-auto pr-2 custom-scrollbar">
                        ...
                        {leads.length === 0 && searchQuery && <p className="text-[10px] text-center text-slate-400 py-4 font-bold uppercase">No Matches Found</p>}
                      </div>
                    </div>
                  ) : (
...
<div className="space-y-2">
 ...
 const slotTimeLabel = formatClinicTime(slot.startTime, wizardClinic?.timezone);
 return (
 <div
 key={idx}
 onClick={() => setWizardTime(slotTimeLabel)}
 className={`py-2 text-center text-[10px] font-black rounded-lg border-2 cursor-pointer transition-all ${wizardTime === slotTimeLabel
 ...
 <p className="text-[11px] font-black text-white/80 uppercase">{salespersons.find(s => s.id === (wizardProviderId || user?.id))?.firstName || user?.firstName}</p>
 </div>
 <div className="text-right">
