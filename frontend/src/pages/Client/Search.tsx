        ...
        <div className={`w-full ${showDesktopMap ? 'lg:w-[55%] xl:w-[60%]' : 'lg:w-[100%]'} transition-all duration-300 p-4 md:p-6 lg:p-8 min-h-screen pb-24`}>
          ...
          {!query && (
            <div className="flex gap-2 overflow-x-auto mb-8 pb-2 no-scrollbar">
              {['Botox', 'Dermal Fillers', 'Laser Hair Removal', 'Chemical Peel'].map(s => (
                <button key={s} className="px-4 py-2 bg-gray-50 hover:bg-gray-100 rounded-full text-sm font-bold text-gray-700 whitespace-nowrap border border-gray-100 transition-colors"
                  onClick={() => handleSearch({ query: s })}>
                  {s}
                </button>
              ))}
            </div>
