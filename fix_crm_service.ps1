...
$pattern = "(?s)async seedMockCrmData.*"
$replacement = @"
  async seedMockCrmData(): Promise<any> {
    ...
    if (agents.length > 0) {
      ...
      for (const [f, l] of leadNames) {
        ...
        // Create some calls for these leads
        for (let i = 0; i < 3; i++) {
      ...
      // 4. Create Appointments (for revenue)
      if (services.length > 0 && clinics.length > 0) {
        for (let i = 0; i < 50; i++) {
          ...
          const apt = this.appointmentsRepository.create({
            ...
            clientId: agents[0].id, // Reuse agent for simplicity in mock
            bookedById: agent.id,
            startTime: new Date(new Date().getTime() - (Math.random() * 30 * 24 * 60 * 60 * 1000)), // up to 30 days ago
            endTime: new Date(),
